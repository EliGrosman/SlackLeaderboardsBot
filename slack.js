...
function Slack(ACCESS_TOKEN, SIGNING_SECRET, VERIFICATION_TOKEN) {
  ...
  axios.defaults.headers.common = this.config;
}
...
Slack.prototype.sendMessage = function sendMessage(text, channelID, callback) {
  ...
  let message = {
   ...
   axios.post(`${apiUrl}/chat.postMessage`, qs.stringify(message), { headers: { authorization: `Bearer ${this.ACCESS_TOKEN}` } })
   .then(function(){
...
Slack.prototype.sendEphemeral = function sendEphemeral(text, channelID, userID, callback) {
  if(!this.isInit) {
   ...
   axios.post(`${apiUrl}/chat.postEphemeral`, qs.stringify(message), this.config)
   .then(function(res){
     console.log(res)
    if (typeof callback === "function") {
      callback();
